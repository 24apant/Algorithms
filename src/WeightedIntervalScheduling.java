...
public class WeightedIntervalScheduling {
    ...
    public int getOptimalSolution() {
        ...
        // if first is not included, check remaining
        // if first is included, check all except any overlapping

        int[] solutions = new int[intervals.length+1];
        solutions[0] = 0;
        solutions[1] = intervals[0].value;

        for(int i = 1; i < intervals.length; i++){
            // check if the intervals would be better if this was included or excluded, and take the better one
            int included = intervals[i].value;

            // if there is a latest non-overlapping interval, update
            if (latestNonOverlapping(i) != -1){
                included += solutions[latestNonOverlapping(i)+1];
            }
            int notIncluded = solutions[i];
            solutions[i+1] = Math.max(included, notIncluded);
        }
