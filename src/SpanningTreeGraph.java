...
public class SpanningTreeGraph {
    ...
    public SpanningTreeGraph prim(String source) {
        ...
        // YOUR CODE HERE
        WeightedVertex curr = vertices.get(source);
        PriorityQueue<WeightedEdge> pq = new PriorityQueue<>();
        HashSet<WeightedVertex> visited = new HashSet<>();
        visited.add(curr);

        for(WeightedEdge e: curr.edges){
            pq.offer(e);
        }
        while(!pq.isEmpty()){
            WeightedEdge e = pq.poll();
            if (!visited.contains(e.destination)){
                // add edges of destination node to the pq IF its destination is not in set
                spanningTree.addEdge(e);
                visited.add(e.destination);
                for(WeightedEdge w: e.destination.edges){
                    pq.offer(w);
                }
            }
        }
    ...
    public SpanningTreeGraph kruskal() {
        ...
        LinkedList<WeightedEdge> unsortedEdges = getAllEdges();

        for(WeightedEdge e: unsortedEdges){
            edges.offer(e);
        }
        ...
        while(edges.size() != 0){
            ...
            // add source and dest
            spanningTree.addEdge(minEdge);

            if(spanningTree.hasCycle(minEdge.source.label)) {
                spanningTree.removeEdge(minEdge.source.label, minEdge.destination.label);
            }
