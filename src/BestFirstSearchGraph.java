...
public class BestFirstSearchGraph {
    ...
    public int bestFirstSearch(String source, String destination) {
        ...
        while(!queue.isEmpty()){
            ...
            GridVertex v = vertices.get(pqNode.label);
            System.out.println(v.label);
            set.add(v.label);


            for(WeightedEdge e: v.edges){
                if(!set.contains(e.destination.label)){

                    if(dist.get(e.destination.label) != null && dist.get(e.destination.label) > dist.get(v.label) + e.weight){
                        dist.put(e.destination.label, dist.get(v.label)+e.weight);
                    }
                    else if (dist.get(e.destination.label) == null){
                        dist.put(e.destination.label, dist.get(v.label)+e.weight);
                    }

                    if(Objects.equals(e.destination.label, destination)){
                        System.out.println(dist);
                        return dist.get(v.label) + e.weight;

                    }
                    queue.add(new PQNode(e.destination.label, manhattanDistance(e.destination.label, destination)));

//                    set.add(e.destination.label);
                }
            }
        }
        System.out.println(dist);
